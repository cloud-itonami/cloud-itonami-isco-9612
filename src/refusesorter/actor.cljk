(ns refusesorter.actor
  "RefuseSorterActor — the ISCO-08 9612 refuse-sorting-facility
  ...
  run = one facility-coordination operation request (intake -> advise ->
  ...
  the itonami actor shape, extended with a second, independent
  hazardous-material-handling hazard-scope dimension.
  ...
  The unconditional invariant: the Refuse Sorter Advisor can never
  directly commit an operating record the RefuseSorterGovernor refuses,
  and can never make this actor finalize a sorting-operation-execution
  decision or a facility-safety-clearance decision (e.g. declaring a
  facility cleared for safety), and can never override a facility safety
  supervisor's judgment — every commit-record! call is gated behind
  `:decide`, and all three scope-excluded decision classes are a
  permanent hard block regardless of disposition path. This actor
  coordinates FACILITY SCHEDULING/LOGISTICS ONLY — it never sorts waste
  materials or performs facility labour work itself and never makes a
  facility-safety-clearance decision itself."
...
(defn build-graph
  "Build a compiled RefuseSorterActor graph. `store` implements
  `refusesorter.store/Store`. `advisor` implements
  `refusesorter.advisor/Advisor` (defaults to `mock-advisor`).
